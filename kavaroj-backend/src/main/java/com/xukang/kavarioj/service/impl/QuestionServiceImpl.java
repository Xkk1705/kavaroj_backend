...
import com.xukang.kavarioj.mapper.QuestionMapper;
...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question>
    ...
    public void validQuestion(Question question, boolean add) {
        ...
        // 创建时，参数不能为空
        if (add) {
            ThrowUtils.throwIf(StringUtils.isAnyBlank(title, content, tags, answer, judgeCase), ErrorCode.PARAMS_ERROR);
        }
        // 有参数则校验
        if (StringUtils.isNotBlank(title) && title.length() > 80) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "标题过长");
        }
        if (StringUtils.isNotBlank(answer) && answer.length() > 80) {
    ...
    public QueryWrapper<Question> getQueryWrapper(QuestionQueryRequest questionQueryRequest) {
        ...
        List<String> tagList = questionQueryRequest.getTagsList();
        List<String> judgeCaseList = questionQueryRequest.getJudgeCaseList();
        ...
        if (CollectionUtils.isNotEmpty(judgeCaseList)) {
            for (String judgeCase : judgeCaseList) {
                queryWrapper.like("tags", "\"" + judgeCase + "\"");
            }
        }
